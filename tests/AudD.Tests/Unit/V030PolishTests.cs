...
public class V030PolishTests_SourceGenContext
{
    ...
    [Fact]
    public void AudDJsonContext_DeserializeStreamCallbackResult_HandlesTopLevelExtras()
    {
        const string body = """
        {
          "radio_id": 99,
        ...
        """;
        using var doc = JsonDocument.Parse(body);
        var parsed = doc.RootElement.Deserialize(AudDJsonContext.Default.StreamCallbackResult);
        Assert.NotNull(parsed);

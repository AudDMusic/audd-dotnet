...
public class ContractTests
{
    ...
    public void StreamsCallbackResult_Parses()
    {
        ...
        var p = StreamCallbackPayload.Parse(body);
        Assert.True(p.IsResult);
        Assert.Equal(7L, p.Result!.RadioId);
        Assert.Equal(111L, p.Result.PlayLength);
        Assert.Single(p.Result.Results);
        Assert.Equal("Alan Walker, A$AP Rocky", p.Result.Results[0].Artist);
    }
    ...
    public void StreamsCallbackNotification_Parses()
    {
        ...
        Assert.Equal(3L, p.Notification!.RadioId);

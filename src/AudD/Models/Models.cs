...
/// <summary>One song from a streams callback's <c>results</c> array.</summary>
public sealed record StreamCallbackResultEntry
{
    [JsonPropertyName("artist")] public string Artist { get; init; } = "";
    [JsonPropertyName("title")] public string Title { get; init; } = "";
    [JsonPropertyName("score")] public int Score { get; init; }
...
/// <summary>Recognition variant of a streams callback POST body.</summary>
public sealed record StreamCallbackResult
{
    [JsonPropertyName("radio_id")] public long RadioId { get; init; }
    [JsonPropertyName("timestamp")] public string? Timestamp { get; init; }
    [JsonPropertyName("play_length")] public long? PlayLength { get; init; }
    [JsonPropertyName("results")] public IReadOnlyList<StreamCallbackResultEntry> Results { get; init; } = Array.Empty<StreamCallbackResultEntry>();
...
/// <summary>Discriminated wrapper over a callback payload.</summary>
public sealed class StreamCallbackPayload
{
    /// <summary>The recognition body, when this payload is a result.</summary>
    public StreamCallbackResult? Result { get; }

    /// <summary>The notification body, when this payload is a notification.</summary>
    public StreamCallbackNotification? Notification { get; }

    /// <summary>Server-side timestamp on notification payloads.</summary>
    public long? Time { get; }

    /// <summary>Original parsed JSON.</summary>
    public JsonElement RawPayload { get; }

    /// <summary>True iff this payload is a recognition result.</summary>
    public bool IsResult => Result is not null;

    /// <summary>True iff this payload is a notification.</summary>
    public bool IsNotification => Notification is not null;

    internal StreamCallbackPayload(
        StreamCallbackResult? result,
        StreamCallbackNotification? notification,
        long? time,
    ...
    /// <summary>Parse a streams callback POST body.</summary>
    public static StreamCallbackPayload Parse(JsonElement body)
    {
        ...
        if (body.TryGetProperty("notification", out var nf) && nf.ValueKind == JsonValueKind.Object)
        {
            ...
            if (body.TryGetProperty("time", out var t) && t.ValueKind == JsonValueKind.Number)
                time = t.GetInt64();
            return new StreamCallbackPayload(null, notif, time, body);
        }

        StreamCallbackResult? result = null;
        if (body.TryGetProperty("result", out var resultEl) && resultEl.ValueKind == JsonValueKind.Object)
        {
            result = resultEl.Deserialize(AudDJsonContext.Default.StreamCallbackResult);
        }

        return new StreamCallbackPayload(result ?? new StreamCallbackResult(), null, null, body);
    }
}

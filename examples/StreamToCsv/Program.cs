...
try
{
    ...
    // Drain notifications + errors concurrently.
    ...
    try
    {
        await foreach (var match in poll.Matches.WithCancellation(cts.Token).ConfigureAwait(false))
        {
            await WriteMatchAsync(match, csv).ConfigureAwait(false);
        }
    }
    catch (OperationCanceledException) { }
}
...
static async Task WriteMatchAsync(StreamCallbackMatch match, StreamWriter csv)
{
    var receivedAt = DateTime.UtcNow.ToString("yyyy-MM-dd'T'HH:mm:ss.fff'Z'", CultureInfo.InvariantCulture);
    // Write the top match plus any alternatives (which may have a different artist/title).
    var entries = new List<StreamCallbackSong> { match.Song };
    entries.AddRange(match.Alternatives);
    foreach (var entry in entries)
    {
        ...
        await csv.FlushAsync().ConfigureAwait(false);
        Console.WriteLine($"[{receivedAt}] radio_id={match.RadioId}: {entry.Artist} - {entry.Title}");
    }
}

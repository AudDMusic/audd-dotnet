...
Official .NET SDK for [AudD](https://audd.io) — music recognition from a short audio clip, a long audio file, or a live stream.
...
The callback receives JSON; parse it into a typed payload from your webhook handler:

```csharp
app.MapPost("/audd-callback", async (HttpRequest req, AudD.AudD audd) =>
{
    using var doc = await JsonDocument.ParseAsync(req.Body);
    var payload = audd.Streams.ParseCallback(doc.RootElement);

    if (payload.IsResult)
    {
        foreach (var entry in payload.Result!.Results)
        {
            Console.WriteLine($"{entry.Artist} — {entry.Title} score={entry.Score}");
        }
    }
    else if (payload.IsNotification)
    {
        Console.WriteLine($"notification: {payload.Notification!.NotificationMessage}");
    }
    return Results.Ok();
});
```

### Receiving events without a callback URL
...
await foreach (var evt in audd.Streams.LongpollAsync(category, timeout: 30, cancellationToken: ct))
{
    Console.WriteLine(evt); // raw JsonElement — same shape as a callback payload
}
```
...
await foreach (var evt in consumer.IterateAsync(timeout: 30, cancellationToken: ct))
{

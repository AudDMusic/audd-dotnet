...
For longer audio files (full-length songs, short-form videos, podcasts, broadcasts, DJ sets), use `RecognizeEnterpriseAsync(source, limit: ...)` — it returns `IReadOnlyList<EnterpriseMatch>`, one per song detected across the file.
...
Valid `returnMetadata` values: `apple_music`, `spotify`, `deezer`, `musicbrainz`. The corresponding properties (`AppleMusic`, `Spotify`, `Deezer`, `Napster`, `MusicBrainz`) stay `null` when not requested.
...
The typed records cover what AudD documents. To read additional fields the server returns, go through `Extras`:
...
For sending arbitrary form fields the typed parameters don't cover, pass an `extraParameters` dictionary:

```csharp
var result = await audd.RecognizeAsync(
    fileInfo,
    returnMetadata: new[] { "apple_music" },
    extraParameters: new Dictionary<string, string> { ["my_custom_flag"] = "1" });
```
...
await using var poll = await audd.Streams.LongpollAsync(radioId, timeout: 30, cancellationToken: ct);
...
> For recognition, use `RecognizeAsync(...)` (or `RecognizeEnterpriseAsync(...)` for longer audio files). The custom-catalog endpoint adds songs to your *private* fingerprint database so future `RecognizeAsync` calls on your account can identify *your own* tracks.
